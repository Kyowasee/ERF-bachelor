...
# Diabetes UCI Data 
data <- read.csv(file = './data sets/diabetes.csv', header = T) #Luisa data
...
# Diabetes UCI Data 
data <- read.csv(file = './data sets/diabetes-4.csv', header = T) #eigen data 
data <- prepare_diabetes_data(data)
```
...
# Diabetes UCI Data 
data <- read.csv(file = './data sets/combined-1.csv', header = T) #niet random 
data <- prepare_diabetes_data(data)
```
...
erf_diabetes <- ExpertRuleFit(X =X, y=y, Xtest = Xtest, ytest = ytest, 
                             ...
                             optional_linear_terms = opt_linear,
                             s = "lambda.1se")
#Results luisa regels 
#Lambda: 0.010333 
#Number of terms: 47 
#Average rule length: 2.4583 
...
#Classification error: 0.2565 
#Proportion of optional EK in the final model:  0.0000 

#Proportion of EK in the final model (overall):  0.2553 
...
#Classification error: 0.2174 
#Proportion of optional EK in the final model:  0.0263 

#Proportion of EK in the final model (overall):  0.2895
...
```{r}
#ERF only data rules (rulefit)
erf_diabetes_datarules_only <- ExpertRuleFit(X =X, y=y, Xtest = Xtest, ytest = ytest,
                                      confirmatory_expert_rules = conf_rules, 
                                      optional_expert_rules = opt_rules, 
                                      confirmatory_linear_terms = conf_linear,
                                      optional_linear_terms = opt_linear,
                                      expert_only = F, s = "lambda.1se")
```
...
#Classification error: 0.2130 
```



```{r}
#ERF 0.5 penalty factor on data rules 
erf_diabetes_ek_only <- ExpertRuleFit(X =X, y=y, Xtest = Xtest, ytest = ytest,
                                      confirmatory_expert_rules = conf_rules, 
                                      optional_expert_rules = opt_rules, 
                                      confirmatory_linear_terms = conf_linear,
